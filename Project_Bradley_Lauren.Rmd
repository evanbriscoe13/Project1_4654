...
#test
#a[index]
#df <- aggregate(.~svi,data = prostate_clean, FUN = sum)
#df[order(df$age),]
```
...
mykNN <- function(train, test, y_train, y_test, k = 3, weighted = TRUE){
    ...
    # Regression
      ...
      for (i in 1:dim(test)[1]){
        ...
        for (x in seq_along(k)){
          fitted_value <- fitted_value + y_train[index[x]]
        }
        y_hat <- append(y_hat, (1/k)*fitted_value)
      }
  ...
  #  * The value of k used
    ...
    for (i in 1:dim(test)[1]){
      ...
      fitted_value <- 0
      for (x in seq_along(k)){
        fitted_value <- fitted_value + (c[x]*y_train[index[x]])
      }
...
```
With k = 3, the k nearest neighbors is 66.95% accurate.
...
Using 10 of the neighbors looks the best. 
...
```
Using k = 10, there is an accuracy of 72.88%
...
knitr::kable(table)
```
The lease error occurs at k = 3. In addition, the regression lines only look accurate at the begining 

```{r}
dists <- calculateDistances(test_features[1,], train_features)
index <- seq_along(dists)
o <- order(dists, decreasing = FALSE)
# Grab k closest distances and indexes
dists_k <- cbind(dists[o],index[o])[1:5,]
weights <- 1/dists_k[, 1]

c <- weights/sum(weights)
fitted_value <- 0
for (x in seq_along(k)){
  fitted_value <- fitted_value + (c[x]*train_classes[index[x]])
}
y_hat <- append(y_hat, fitted_value)
```
